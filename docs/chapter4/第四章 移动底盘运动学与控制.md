...
其中 $\theta_i$ 是第 $i$ 个轮子的转向角，$\omega_i$ 是第 $i$ 个轮子的自转角速度。把底盘速度转换为四个轮子状态的过程称为逆运动学。

把 G2 底盘看作平面刚体。刚体上任意一点的速度由底盘中心平移速度和绕中心旋转产生的切向速度叠加而成。
...
$$
\boxed{\theta_i=\operatorname{atan2}(v_{iy},v_{ix})}
$$
...
仿真程序必须使用 Isaac Sim 自带的 Python 环境，不要直接使用普通系统 Python。
...
如果机器人完全不动，应首先检查是否使用 `/home/robot/isaac-sim/python.sh` 启动、G2 是否成功加载、底盘关节是否正确找到，以及控制循环中是否依次调用了速度设置、控制更新和物理步进。
