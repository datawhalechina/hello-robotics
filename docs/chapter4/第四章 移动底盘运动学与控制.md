...
> **仿真资源下载：** 本章所需的 `assets` 仿真资源可以从huggingface中的 [Datawhale/g2_assets](https://huggingface.co/datasets/Datawhale/g2_assets/tree/main) 下载。下载并解压后，将完整的 `assets` 文件夹放到项目的 `code` 目录下，最终目录结构应为 `code/assets/`。请不要只复制某个 USD 文件，因为 USD 场景可能继续引用同目录或子目录中的纹理、材质和其他模型资源。
