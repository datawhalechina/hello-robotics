...
"""OmniGraph-based camera ROS2 publishers (RGB, depth, pointcloud, bbox, semantic)."""
...
def publish_pointcloud_from_depth(camera: Camera, step_size: int, topic=""):
    """Publish point cloud from depth via ROS2."""
    render_product = camera._render_product_path
...
def publish_boundingbox2d_loose(camera: Camera, step_size: int, topic=""):
    """Publish 2D loose bounding boxes via ROS2."""
    render_product = camera._render_product_path
...
def publish_boundingbox2d_tight(camera: Camera, step_size: int, topic=""):
    """Publish 2D tight bounding boxes via ROS2."""
    render_product = camera._render_product_path
...
def publish_boundingbox3d(camera: Camera, step_size: int, topic=""):
    """Publish 3D bounding boxes via ROS2."""
    render_product = camera._render_product_path
...
def publish_semantic_segment(camera: Camera, step_size: int, topic=""):
    """Publish semantic segmentation via ROS2 OmniGraph."""
    render_product = camera._render_product_path
    ...
    og.Controller.edit(
        ...
        },
        {
            ...
            og.Controller.Keys.CONNECT: [
                ("OnPlaybackTick.outputs:tick", "publish_semantic.inputs:execIn"),
            ],
        },
    )
