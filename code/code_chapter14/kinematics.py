...
class RightArmKinematics:
    ...
    def _chain(self, joints):
        current = np.eye(4)
        points, axes = [], []
        for origin, axis, angle in zip(self.origins, self.axes, np.asarray(joints), strict=True):
    ...
    def inverse(self, position, rotation=None, initial=None, max_iterations=300) -> IKResult:
        """位置优先的DLS IK。

        桌面抓取时位置是否到达比固定末端朝向更重要，因此默认只求位置；
        ``rotation``参数保留给后续课程扩展，但本章专家不强制锁死姿态。
        """
        del rotation
        if initial is None:
